...
/** Wraps any email body in the shared SproutX branded shell */
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;">
          ...
          <tr>
            <td style="background-color:${BRAND.sidebar};border-radius:16px 16px 0 0;padding:24px 32px;text-align:center;">
              <img src="${base}/logo.png" alt="SproutX" height="36" style="height:36px;object-fit:contain;" />
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:24px 32px;text-align:center;">
              <p style="margin:0 0 8px;font-size:13px;color:${BRAND.textMuted};">
                ...
                <a href="${base}" style="color:${BRAND.primary};text-decoration:none;">SproutX</a>.
              </p>
              <p style="margin:0;font-size:12px;color:${BRAND.textMuted};">
                © ${new Date().getFullYear()} SproutX Technologies Ltd · Nairobi, Kenya
              </p>
            </td>
          </tr>
...
export function emailVerificationTemplate(ctx: {
  ...
  const subject = "Verify your SproutX email address";
  const html = shell(
    ...
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
      Hi ${ctx.userName}, welcome to SproutX! Please verify your email address to activate your account.
    </p>
    ${button("Verify Email Address", ctx.verificationUrl)}
    ${infoBox(`This link expires in <strong>${ctx.expiresInHours} hours</strong>. If you didn't create a SproutX account, you can safely ignore this email.`)}
    ${divider}
    ...
    "Verify your email address to get started with SproutX",
  );
  const text = `Hi ${ctx.userName},\n\nVerify your SproutX email: ${ctx.verificationUrl}\n\nThis link expires in ${ctx.expiresInHours} hours.`;
...
export function passwordResetTemplate(ctx: {
  ...
  const subject = "Reset your SproutX password";
  const html = shell(
    `<h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:${BRAND.textPrimary};">Reset your password</h1>
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
      Hi ${ctx.userName}, we received a request to reset your SproutX password. Click the button below to choose a new password.
    </p>
    ...
    "Reset your SproutX account password",
  );
...
export function welcomeEmailTemplate(ctx: {
  ...
  const subject = "Welcome to SproutX 🌱";
  const features = [
  ...
  const html = shell(
    ...
    <p style="margin:0 0 24px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
      Your SproutX account is ready. You're joining thousands of smart farmers who are growing more efficiently with technology.
    </p>
    ...
    ${infoBox("Need help getting started? Visit our <a href='https://sproutxhub.onrender.com' style='color:${BRAND.primary};'>Help Center</a> or reply to this email and we'll assist you.")}`,
    `Welcome aboard, ${ctx.userName}! Your SproutX account is ready.`,
  );
  const text = `Welcome to SproutX, ${ctx.userName}!\n\nYour account is ready. Get started: ${ctx.dashboardUrl}`;
  return { subject, html, text };
}
...
export function farmInviteTemplate(ctx: {
  ...
  const subject = `${ctx.inviterName} invited you to join ${ctx.farmName} on SproutX`;
  const html = shell(
    `<h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:${BRAND.textPrimary};">You've been invited!</h1>
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
      <strong>${ctx.inviterName}</strong> has invited you to join <strong>${ctx.farmName}</strong> on SproutX as a <strong>${ctx.role}</strong>.
    </p>
    ${button(`Accept Invitation to ${ctx.farmName}`, ctx.inviteUrl)}
    ${infoBox(`This invitation expires in <strong>${ctx.expiresInDays} days</strong>. You will need a SproutX account to accept.`)}
    ${divider}
    <p style="margin:0;font-size:13px;color:${BRAND.textMuted};">
...
export function organizationInviteTemplate(ctx: {
  ...
  const subject = `${ctx.inviterName} invited you to join ${ctx.organizationName} on SproutX`;
  const html = shell(
...
export function platformAnnouncementTemplate(ctx: {
  ...
  const html = shell(
    ...
    ${divider}
    <p style="margin:0;font-size:13px;color:${BRAND.textMuted};">This is a system announcement from the SproutX team.</p>`,
    "Important update from SproutX"
  );
...
export function paymentReminderTemplate(ctx: {
  ...
  const subject = `Action Required: SproutX Subscription Expiring Soon`;
  const html = shell(
    `<h1 style="margin:0 0 8px;font-size:24px;font-weight:700;color:${BRAND.textPrimary};">Subscription Expiring</h1>
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
      Hi ${ctx.userName}, your SproutX <strong>${ctx.planName}</strong> subscription will expire on <strong>${ctx.expiryDate}</strong>.
    </p>
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
    ...
    "Your SproutX subscription is expiring soon"
  );
  const text = `Hi ${ctx.userName},\n\nYour SproutX ${ctx.planName} subscription expires on ${ctx.expiryDate}. Renew for ${ctx.amount}: ${ctx.paymentUrl}`;
  return { subject, html, text };
}
...
export function securityAlertTemplate(ctx: {
  ...
  const html = shell(
    ...
    "Important security alert regarding your SproutX account"
  );
...
export function trialStartedEmailTemplate(ctx: {
  ...
  const html = shell(
    ...
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
      Hi ${ctx.userName}, your <strong>${ctx.trialDays}-day free trial</strong> of the SproutX ${ctx.planName} plan for <strong>${ctx.organizationName}</strong> has officially started!
    </p>
    <p style="margin:0 0 20px;font-size:16px;color:${BRAND.textSecondary};line-height:1.6;">
    ...
    `Your ${ctx.trialDays}-day free trial of SproutX ${ctx.planName} has started.`
  );
  const text = `Hi ${ctx.userName},\n\nYour ${ctx.trialDays}-day free trial of the SproutX ${ctx.planName} plan for ${ctx.organizationName} has started.\n\nManage your subscription here: ${ctx.billingUrl}`;
  return { subject, html, text };
}
