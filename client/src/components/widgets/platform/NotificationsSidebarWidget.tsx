    ...
    <Card className={cn("border shadow-sm bg-white flex flex-col", className)}>
      <div className="p-3 border-b flex items-center justify-between">
        ...
        <Link href="/notifications">
          <span className="text-[11px] font-bold text-muted-foreground hover:text-rose-600 cursor-pointer">View All</span>
        </Link>
      </div>
        ...
        {recent.length === 0 ? (
          ...
          recent.map(n => (
            <div
              key={n.id}
              className={cn(
                "flex items-start gap-3 px-4 py-3 border-b border-slate-50 last:border-0 transition-colors cursor-pointer hover:bg-muted",
                !n.isRead ? "bg-rose-50/30" : ""
              )}
            >
              <div className={cn("w-2 h-2 rounded-full mt-1.5 shrink-0", !n.isRead ? "bg-rose-500" : "bg-slate-300")} />
              <div className="flex-1 min-w-0">
                <p className="text-[12px] font-medium leading-tight text-muted-foreground truncate">{n.title}</p>
                {n.message && (
