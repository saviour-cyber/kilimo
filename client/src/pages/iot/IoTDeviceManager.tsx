    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-500">Register and monitor all connected IoT devices</p>
        </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-16 text-center flex flex-col items-center gap-4">
            ...
            <div>
              <p className="font-medium text-slate-600">No devices registered yet</p>
              <p className="text-sm text-slate-400 mt-1">Register your first simulated device to start generating telemetry</p>
            </div>
          ...
          devices.map(device => {
            ...
            return (
              <div key={device.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                {/* Device Header Row */}
                <button
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg
                      ...
                      {DEVICE_TYPE_LABELS[device.deviceType]?.split(" ")[0] ?? "📡"}
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-500">
                        ...
                        {device.model && <><span>•</span><span>{device.model}</span></>}
                      </div>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-50 px-6 py-5">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <span className="text-xs text-slate-400">Protocol: <strong>{device.protocol}</strong></span>
                      {device.firmwareVersion && (
                        <span className="text-xs text-slate-400">FW: <strong>{device.firmwareVersion}</strong></span>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {sensors.map(sensor => {
                        ...
                        return (
                          <div key={sensor.id} className="bg-slate-50 rounded-xl p-3">
                            ...
                            <div className="flex items-baseline gap-1 mt-1">
                              {reading && reading.value != null ? (
                                <>
                                  <span className="text-lg font-bold text-slate-800">{reading.value.toFixed(1)}</span>
