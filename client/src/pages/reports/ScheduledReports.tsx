    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Manage recurring automated reports</p>
        </div>
      </div>
              ...
              <div key={schedule.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                <div>
                  <h3 className="font-semibold text-slate-800 flex items-center gap-2">
                    {schedule.name}
                    {!schedule.isActive && (
                      <span className="text-[10px] uppercase font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">Paused</span>
                    )}
                  </h3>
